<aside class="left-sidebar">
  <div>
    <div class="close-btn d-xl-none d-block sidebartoggler cursor-pointer" id="sidebarCollapse">
      <i class="ti ti-x fs-6"></i>
    </div>

    <nav class="sidebar-nav scroll-sidebar" data-simplebar="">
      <ul id="sidebarnav">
        <!-- Menu Utama -->
        <li class="nav-small-cap">
        </li>

        <li class="sidebar-item">
          <a class="sidebar-link active" href="{{ url('/') }}">
            <i class="ti ti-home"></i>
            <span class="hide-menu">Dashboard</span>
          </a>
        </li>

        <!-- Master Data -->
        <li class="sidebar-item">
          <a class="sidebar-link has-arrow" href="#" aria-expanded="false">
            <div class="d-flex align-items-center gap-3">
              <span class="d-flex"><i class="ti ti-database"></i></span>
              <span class="hide-menu">Master Data</span>
            </div>
          </a>
          <ul aria-expanded="false" class="collapse first-level">
            <li class="sidebar-item">
              <a class="sidebar-link" href="{{ route("master.pegawai") }}">
                <div class="d-flex align-items-center gap-3">
                  <div class="round-16 d-flex align-items-center justify-content-center">
                    <i class="ti ti-users"></i>
                  </div>
                  <span class="hide-menu">User / Pegawai</span>
                </div>
              </a>
            </li>
            <li class="sidebar-item">
              <a class="sidebar-link" href="{{ route("master.tim") }}">
                <div class="d-flex align-items-center gap-3">
                  <div class="round-16 d-flex align-items-center justify-content-center">
                    <i class="ti ti-user-check"></i>
                  </div>
                  <span class="hide-menu">Tim Penanggung Jawab</span>
                </div>
              </a>
            </li>
            <li class="sidebar-item">
              <a class="sidebar-link" href="{{ route("master.transportasi") }}">
                <div class="d-flex align-items-center gap-3">
                  <div class="round-16 d-flex align-items-center justify-content-center">
                    <i class="ti ti-car"></i>
                  </div>
                  <span class="hide-menu">Jenis Transportasi</span>
                </div>
              </a>
            </li>
          </ul>
        </li>

        <!-- Perjalanan Dinas -->
        <li class="sidebar-item">
          <a class="sidebar-link has-arrow" href="#" aria-expanded="false">
            <div class="d-flex align-items-center gap-3">
              <span class="d-flex"><i class="ti ti-plane"></i></span>
              <span class="hide-menu">Perjalanan Dinas</span>
            </div>
          </a>
          <ul aria-expanded="false" class="collapse first-level">
            <li class="sidebar-item">
              <a class="sidebar-link" href="{{ url('/perjalanan') }}">
                <div class="d-flex align-items-center gap-3">
                  <div class="round-16 d-flex align-items-center justify-content-center">
                    <i class="ti ti-circle"></i>
                  </div>
                  <span class="hide-menu">Daftar Perjalanan</span>
                </div>
              </a>
            </li>
            <li class="sidebar-item">
              <a class="sidebar-link" >
                <div class="d-flex align-items-center gap-3">
                  <div class="round-16 d-flex align-items-center justify-content-center">
                    <i class="ti ti-circle"></i>
                  </div>
                  <span class="hide-menu">Input Perjalanan</span>
                </div>
              </a>
            </li>
          </ul>
        </li>

        <li class="sidebar-item">
          <a class="sidebar-link" href="{{ url('/laporan') }}">
            <i class="ti ti-chart-bar"></i>
            <span class="hide-menu">Laporan</span>
          </a>
        </li>

        <li><span class="sidebar-divider lg"></span></li>


      </ul>
    </nav>
  </div>
</aside>
